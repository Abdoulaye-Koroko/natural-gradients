...
from utils.optim_utils import power_svd_mlp,power_svd_cnn,vec,lanczos_mlp,lanczos_cnn



class Lanczos(BaseOptimizer):
    ...
    def _compute_covs(self, group, state):
        ...
        if self._iteration_counter == 0:
            q = torch.rand((gy.shape[0],1),device = self.device)
            q = vec(torch.mm(q,q.t()))
            if group["layer_type"]=="Linear":
                
                state['xxt1'],state['ggt1'],state['xxt2'],state['ggt2'] = lanczos_mlp(x.t(),gy.t(),q,self.K,self.epsilon)
                ...
                state['xxt1'],state['ggt1'],state['xxt2'],state['ggt2'] = lanczos_cnn(x,gy,q,self.K,self.epsilon,r)
            ...
            if group["layer_type"]=="Linear":
                ...
                xxt1,ggt1,xxt2,ggt2 = lanczos_mlp(x.t(),gy.t(),q,self.K,self.epsilon)
                ...
                xxt1,ggt1,xxt2,ggt2 = lanczos_cnn(x,gy,q,self.K,self.epsilon,r)
