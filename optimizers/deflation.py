...
class Deflation(BaseOptimizer):
    ...
    def _compute_covs(self, group, state):
        ...
        if self._iteration_counter == 0:
            v1 = torch.rand((gy.shape[0],1),device=self.device)
            v1 = torch.mm(v1,v1.t())
            v2 = torch.rand((gy.shape[0],1),device=self.device)
            v2 = torch.mm(v2,v2.t())
            if group["layer_type"]=="Linear":
                
                state['xxt1'],state['ggt1'] = power_svd_mlp(x.t(),gy.t(),vec(v1),None
                  ,None,self.epsilon,self.max_iter,self.method)
                
                state['xxt2'],state['ggt2'] = power_svd_mlp(x.t(),gy.t(),vec(v2),state['xxt1']
                ...
                state['xxt1'],state['ggt1'] = power_svd_cnn(x,gy,v1,None,
                  None,self.epsilon,self.max_iter,r,self.method)
                
                state['xxt2'],state['ggt2'] = power_svd_cnn(x,gy,v2,state["xxt1"],
            ...
            if group["layer_type"]=="Linear":
                xxt1,ggt1 = power_svd_mlp(x.t(),gy.t(),vec(v1),None
                  ,None,self.epsilon,self.max_iter,self.method)
                xxt2,ggt2 = power_svd_mlp(x.t(),gy.t(),vec(v2),xxt1
                  ,ggt1,self.epsilon,self.max_iter,self.method)
            elif group["layer_type"]=="Conv2d":
                x = x.t().contiguous().view(self.batch_size,T,x.shape[0]).permute(0,2,1)
                gy = gy.t().contiguous().view(self.batch_size,T,gy.shape[0]).permute(0,2,1)
                xxt1,ggt1 = power_svd_cnn(x,gy,v1,None,
                  None,self.epsilon,self.max_iter,r,self.method)
                xxt2,ggt2 = power_svd_cnn(x,gy,v2,xxt1,
                  ggt1,self.epsilon,self.max_iter,r,self.method) 

            state['xxt1'] = rho*state['xxt1']+(1-rho)*xxt1

            state['ggt1'] = rho*state['ggt1']+(1-rho)*ggt1
