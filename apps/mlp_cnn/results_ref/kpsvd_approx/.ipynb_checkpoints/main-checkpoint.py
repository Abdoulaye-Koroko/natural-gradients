...
auto_parser.add_argument('--kl', type=float, default=1e-3,
                    help='kl clipping')
...
def grid_search(args):
    ...
    #npy_dir = os.path.join(root,"outputs_"+args.init_params,tensorboard_name)
    ...
    if args.optim in ["kfac","kpsvd","kfac_cor","lzs","kpsvd_d"]:
        if not args.two_level or args.dd:
            ...
            weight_decays = [1e-3]
            momentums = [0.9]
            two_level_coffs = [1,1e-1,1e-2,1e-3,1e-4]
            ...
            for params in itertools.product(*list_params):
                ...
                try:
                    loss,result = train_function(args)
                    #np.save(npy_dir+"_"+args.R0[:4]+"_"+str(twc)+'.npy', residuals)
                    if loss<best_loss:
                        best_loss = loss
